...
1. **`_MMU_SAVE_POSITION` / `_MMU_RESTORE_POSITION` in PAUSE/RESUME are REAL HH
   plumbing — keep them (CORRECTED 2026-07-14; the original version of this item
   said to delete them, which was wrong).**
   They are defined in HH's `mmu_sequence.cfg` (lines ~216/240 in v3.42) and
   registered live. The original finding missed them because `mmu/base/` files
   are **absolute symlinks** into the HH install (`/home/pi/addon-packages/Happy-Hare/`)
   — dangling on a local checkout, and skipped by `grep -r` (lowercase `-r` does
   not follow symlinks; use `grep -R`).
   - Detect (per custom PAUSE/RESUME on an HH printer): the MMU-enabled branch of
     PAUSE should call `_MMU_SAVE_POSITION` before `PAUSE_BASE`, and RESUME should
     call `_MMU_RESTORE_POSITION` before `RESUME_BASE` (after any reheat — its
     unretract needs `can_extrude`). Missing calls = gap vs the documented HH
     client-macro pattern (`config/optional/client_macros.cfg`).
   - Apply: add the calls, guarded on `printer.mmu.enabled`. They are idempotent
     (save records only the first position until cleared), and HH's Python PAUSE
     wrapper also fires them when enabled — so adding them is always safe, and
     they cover paths the wrapper misses.
   - **Verification rule this taught us:** never trust a repo grep to prove a
     macro undefined. Confirm against the live registry:
     `curl -s localhost:7125/printer/gcode/help` on the printer.
...
1. Greps must be clean: `G4 S` (in .cfg), `reason_code`, `RETRIES=8`;
   `ALL_FANS_OFF` has no callers inside COOLDOWN_SEQUENCE; exactly one
   COOLDOWN_SEQUENCE caller passing the new QUICK semantics. On HH printers,
   PAUSE/RESUME DO call `_MMU_SAVE_POSITION`/`_MMU_RESTORE_POSITION` in their
   MMU-enabled branches (see corrected item 1 — a clean grep here is a bug,
   not a pass). Use `grep -R`, never `-r`, anywhere `mmu/base/` symlinks are
   in scope, and confirm undefined-macro claims against
